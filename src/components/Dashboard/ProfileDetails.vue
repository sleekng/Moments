...
export default {
  ...
  methods: {
    ...
    copyLink() {
      navigator.clipboard
        .writeText(`${this.$website}/${this.user.username}`)
        .then(() => {
    ...
    shareToEmail() {
      const subject = encodeURIComponent(
        `Check out this profile: ${this.user.username}`
      );
      const body = encodeURIComponent(`${this.$website}/${this.user.username}`);
      window.location.href = `mailto:?subject=${subject}&body=${body}`;
    },
    shareToWhatsApp() {
      const text = encodeURIComponent(
        `Check out this profile: ${this.$website}/${this.user.username}`
      );
    ...
    shareToFacebook() {
      const url = encodeURIComponent(`${this.$website}/${this.user.username}`);
      window.open(
