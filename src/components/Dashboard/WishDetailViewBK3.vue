      ...
      <div class="flex items-center gap-1 mt-1">
        ...
        <span class="text-[#3b3f46] text-sm leading-tight">+23478362739</span>
          ...
          <div v-if="wish.status == 'reserved'">
           ...
           <!-- Show edit and mark as received buttons only if user owns the wish -->
           

           <div v-if="!isDashboard" >
             <div v-if="isWishOwner" class="pt-8 absolute bottom-0 flex justify-between w-full space-x-4">
               <button 
                 @click.stop="$emit('editWish', wish)" 
                 class="flex-1 px-8 py-3 bg-gray-200 text-gray-800 rounded-full hover:bg-gray-300"
               >
                 Edit
               </button>
               <button 
                 @click="markAsReceived" 
                 class="flex-1 px-8 py-3 bg-primaryColor text-white rounded-full hover:shadow-lg"
               >
                 Mark as received
               </button>
             </div>
          <!-- Show different actions for non-owners -->
          <div v-else class="pt-8 flex justify-between w-full space-x-4 absolute bottom-0">
                <div class="flex-1"></div>
                <button 
                  class="flex-1 px-8 py-3 bg-primaryColor text-white rounded-full hover:shadow-lg"
             ...
             <!-- Not Used yet -->
            <div v-if="false"  class="pt-8 absolute bottom-0 flex justify-between w-full space-x-4 ">
            ...
            <!-- Not Used yet -->
            <div v-if="false"  class="pt-8 flex justify-between w-full space-x-4 absolute bottom-0">
              <div class="flex-1"></div>
              <button class="flex-1 px-8 py-3 bg-primaryColor text-white rounded-full hover:shadow-lg">
                Mark as Fulfilled
              </button>
...
<script>
import { wishOwnerMixin } from '@/mixins/wishOwnerMixin';
...
export default {
  ...
  methods: {
    ...
    async markAsReceived() {
      try {
        await this.$axios.put(`${this.$baseURL}/wishes/${this.wish.id}`, 
          { received: true },
          {
        ...
        this.$emit('wishUpdated', this.wish);
      } catch (error) {
    ...
    async requestAddress() {
      try {
        await this.$axios.get(`${this.$baseURL}/wishes/${this.wish.id}/address`, {
          headers: { 'Authorization': `Bearer ${localStorage.getItem('authToken')}` }
        });
